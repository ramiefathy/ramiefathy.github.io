 ...
 * This layer narrows the default graph to source-explicit or curator-confirmed
...
(() => {
  ...
  const DEFAULT_ORIGINS = new Set(['source-explicit', 'curator-confirmed'])
  ...
  const VASCULITIS_CONDITIONS = [
    {
      ...
      pearls: ['ANCA specificity and clinical phenotype both matter.', 'Avacopan targets C5aR1 but is not induction monotherapy.', 'Organ-threatening renal or pulmonary disease requires prompt combination therapy.'],
    ...
    {
      id: 'gca', name: 'Giant-cell arteritis / large-vessel vasculitis', abbr: 'GCA/LVV', group: 'Vasculitis', color: '#e11d48',
    ...
    },
    {
      ...
      one_liner: 'Deposited IgA, cryoglobulin or other immune complexes activate Fc receptors and classical complement in small vessels.',
      ...
      flow: ['Persistent antigen or clone', 'Immune-complex formation', 'Vascular deposition', 'Classical-complement activation', 'Neutrophilic small-vessel injury', 'Purpura, ulcers or glomerulonephritis'],
      refs: ['R28', 'R53']
    }
  ]
  ...
  function splitVasculitisEndotypes() {
    ...
    const steroidManifestations = {
      aav: 'Active organ-threatening or nonsevere AAV as part of an induction regimen',
      egpa: 'Acute eosinophilic or vasculitic organ inflammation',
      gca: 'Immediate treatment of active cranial or large-vessel arteritis',
      immune_complex_vasculitis: 'Organ-threatening immune-complex vasculitis after cause-directed assessment'
    }
    ...
    DATA.effects.forEach(effect => {
      ...
      if (effect.med === 'systemic_gc') {
        ...
        VASCULITIS_ENDOTYPES.forEach(condition => rewrittenEffects.push({ ...clone(effect), condition, manifestations: steroidManifestations[condition], caveat: `${effect.caveat || ''} Endotype-specific steroid-sparing therapy and cause-directed management remain required.`.trim(), endotypeScope: condition }))
        return
      }
  ...
  function sanitizeManifestationLinks() {
    ...
    for (const original of DATA.manifestationLinks) {
      ...
      if (curatorDecision) {
        link.relationship = 'Curator-confirmed'
        link.relationOrigin = 'curator-confirmed'
        link.relationMeaning = 'contributory'
        link.mappingConfidence = 'moderate'
        link.curationStatus = 'reviewed'
        link.sourceAssertion = pathway.manifestations
        link.sourceSpan = ''
        link.curatorDecision = curatorDecision
        link.rationale = `Curator-confirmed mapping: ${curatorDecision} Parent pathway statement: “${pathway.manifestations}.”`
        link.defaultVisible = true
        delete link.supportingMedication
        active.push(link)
        continue
    ...
    DATA.meta.manifestationLinkMethod = 'Default pathway–manifestation edges require an exact source span or a documented curator decision. Lexical, organ-domain and treatment-response inferences are quarantined as optional exploratory hypotheses. Treatment response never establishes disease-pathway causality. Rejected mappings remain auditable but are not rendered.'
    DATA.meta.relationshipContract = {
      ...
      relationOrigin: ['source-explicit', 'curator-confirmed', 'domain-inferred', 'therapy-informed', 'lexical-inferred', 'canonical-background'],
  ...
  function installLayerControls() {
    ...
    EDGE_STYLE.drives.label = 'reviewed pathway–phenotype relationship'
    ...
    if (axisNote && !$('#networkEvidenceBoundary')) {
      axisNote.insertAdjacentHTML('afterend', `<div id="networkEvidenceBoundary" class="network-evidence-boundary" role="status" aria-live="polite"><b>Default scientific view:</b> source-explicit and curator-confirmed phenotype links only. Exploratory mappings and canonical-background rules are off.</div>`)
    }
  ...
  async function runLayerState({ exploratoryOn, canonicalOn }) {
    ...
    if (boundary) {
      ...
      heading.textContent = exploratoryOn || canonicalOn ? 'Expanded exploratory view:' : 'Default scientific view:'
