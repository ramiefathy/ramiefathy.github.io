...
let websocketUrl = DEFAULT_WEBSOCKET_URL;
...
if (websocketUrlInput) {
    websocketUrlInput.addEventListener('change', (event) => {
...
function displayStatusMessage(message, targetId = 'transcriptionOutput') {
    const area = document.getElementById(targetId);
    if (!area) return;
    const note = document.createElement('p');
    note.className = 'text-xs text-slate-500 italic my-1';
...
function reconnectWebSocket() {
    try {
        ...
        console.warn('Unable to close websocket before reconnecting', err);
    }
    connectWebSocket();
}
...
function connectWebSocket() {
    ...
    let urlForConnection = websocketUrl;
    try {
        const parsed = new URL(websocketUrl);
        if (sessionToken) {
            parsed.searchParams.set('token', sessionToken);
        }
        urlForConnection = parsed.toString();
    } catch (_) {
        if (sessionToken) {
            const separator = websocketUrl.includes('?') ? '&' : '?';
            urlForConnection = `${websocketUrl}${separator}token=${encodeURIComponent(sessionToken)}`;
        }
    }

    websocket = new WebSocket(urlForConnection);
