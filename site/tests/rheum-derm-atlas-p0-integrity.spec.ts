...
test.describe('Rheum–Derm Atlas P0 scientific-integrity remediation', () => {
  test('limits defaults to explicit synthesis matches and preserves an auditable quarantine', async ({ page }) => {
    ...
    expect(audit.defaultOrigins.sort()).toEqual(['source-explicit'])
  ...
  test('splits vasculitis into non-propagating endotype contexts', async ({ page }) => {
    ...
    expect(scopes.aav.effects).not.toContain('avacopan')
    expect(scopes.aav.effects).toContain('systemic_gc')
    expect(scopes.aav.effects).not.toContain('mepo')
    expect(scopes.egpa.pathwayFamilies).toContain('Eosinophilic')
    expect(scopes.egpa.effects).toEqual(expect.arrayContaining(['mepo', 'benra']))
    ...
    expect(scopes.gca.effects).toEqual(expect.arrayContaining(['tociliz', 'jaki']))
  ...
  test('uses synthesis-versus-exploratory relationship language throughout the manifestation workbench', async ({ page }) => {
    const runtime = watchRuntime(page)
    await openAtlas(page)
    ...
    await expect(page.locator('#manifestMatrixLegend')).toContainText('Synthesis-explicit (not validated)')
    ...
    await expect(page.locator('#manifestCellInfo')).toContainText('match the embedded synthesis')
    ...
    await expect(page.locator('#manifestTableMode option')).toHaveCount(2)
    runtime.assertClean()
  })
  ...
  test('keeps exploratory and canonical-background hypotheses opt-in and reversible', async ({ page }) => {
    ...
    await expect(page.locator('#networkEvidenceBoundary')).toContainText('Default synthesis view')
