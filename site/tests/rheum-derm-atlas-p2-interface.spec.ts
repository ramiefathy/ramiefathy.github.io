...
async function openAtlas(page: import('@playwright/test').Page, suffix = '') {
  ...
  await page.goto(`${atlasPath}${suffix}`)
...
test.describe('Rheum–Derm Atlas P1/P2 governed interface', () => {
  ...
  test('3D exposes epistemic denominators and a simplified non-quantitative grammar', async ({ page }) => {
    ...
    await expect(page.locator('#networkEpistemicDenominator')).toContainText('Visible eligible')
  ...
  test('expanded provenance inspector renders independent dimensions and conflicts', async ({ page }) => {
    ...
    const relation = await page.evaluate(() => {
      ...
      const nodeInfo = document.querySelector('#nodeInfo')
      if (nodeInfo) nodeInfo.textContent = [row.relationshipId, row.manifestation, row.feature, row.pathwayKey, row.condition].filter(Boolean).join(' ')
      ;(window as any).__ATLAS_P2__.refresh()
  ...
  test('exports only the current governed visible subset', async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(/^rheum-derm-atlas-visible-.*\.json$/)
    const path = await download.path()
    expect(path).toBeTruthy()
    const payload = JSON.parse(await import('node:fs/promises').then(fs => fs.readFile(path!, 'utf8')))
    expect(payload.schema).toBe('rheum-derm-atlas-visible-subset-v1')
...
test('touch/mobile defaults to provenance and preserves target size and reflow', async ({ browser }) => {
  ...
  const sizes = await page.locator('.atlas-task-card, .network-share-controls .btn, .network-nondrag-controls .btn').evaluateAll(elements => elements.map(element => {
