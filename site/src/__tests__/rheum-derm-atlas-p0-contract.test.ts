...
describe('Rheum–Derm Atlas P0 scientific-integrity source contract', () => {
  ...
  it('replaces obsolete manifestation-map labels with reviewed and exploratory provenance language', () => {
    ...
    expect(remediation).toContain("['Synthesis-explicit', 'Synthesis-explicit']")
    expect(remediation).toContain("['Editorial hypothesis', 'Editorial hypothesis']")
    expect(remediation).toContain("['Therapy-informed hypothesis', 'Therapy-informed hypothesis']")
    expect(index).toContain("['Directly named','Synthesis-explicit'].includes(link.relationship)")
    expect(index).toContain('Synthesis-explicit (not validated)')
