...
- **Field Console design contract** (`site/src/security/site-design-contract.test.ts`): the site is a single-theme **dark** design — ground `#0b0e13`, text `#f2f4f6`, and **coral `#ff6b4a` as the only accent hue**. "Live" state is signalled by form (pulsing dot, blinking caret, streaming text), never by a second colour; a phosphor-green accent is explicitly banned because green + coral clashed. Type is Fraunces (display) / Bricolage Grotesque (UI) / Space Mono (data), all **self-hosted** via `site/public/apps/shared/legacy-fonts.css` — no Google Fonts CDN in `src/`. The retired "atlas" theme's ornament (decorative barcodes, plate numbering) and verbiage (volume references, "a working atlas", map-coordinate language) are grep-banned. Legacy token names (`--bone`, `--ink`, `--terracotta`, …) survive as **role-preserving aliases** over the new palette so the ~2400 downstream rules in `global.css` keep resolving; don't delete them.
