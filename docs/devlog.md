...
## [2026-09-07] PR #185 follow-up: structured profile and roadmap contracts

**Reconciliation.** PR #185 was already merged: final PR head `66684c44eb77ab0fad0e9411a5b32389ac5bb073`, merge `515b3699c20601c3657c413df7fbd64353f22be3`. The follow-up starts from current master `b8232d73ffbc376fdc3f9948a388546eceb693a4`. All three unresolved inline findings and the latest review's outside-diff caption correction still applied there.

**Changes.** About now projects training and service rows from `timeline.json`, with explicit role/tag fields and separate House Staff Council and curriculum-service records. Existing About-page wording and displayed dates were preserved in structured content, not independently revalidated. Header affiliation, status identity, city, and clock timezone come from `profile.json`; the footer and About city use the same city projection. The training caption is now "Training and degrees, in reverse chronological order."

The roadmap consistently assigns VLM benchmark and MCQ payloads and schemas to immutable `v<semver>` paths, with `latest.json` only a mutable pointer; R2 retains the same versioning and manifest contract. Workstream 7 sourcing moves from week 26 to an unscheduled post-v1 Phase 1. These are planning corrections, not implementation of the proposed APIs or viewer.

**Regression coverage.** Eleven profile/projection cases validate schema, chronology, role preservation, changed source fields, and non-mutation; one mocked-profile SSR case rejects hardcoded affiliation/city; four content-contract cases guard roadmap paths, deferral, and template wiring. Three new browser cases compare every rendered CV row with source data, exercise desktop/mobile hydration with a different visitor timezone, and verify JavaScript-disabled HTML. During broader qualification, existing palette tests could send Tab or Escape before the input's requestAnimationFrame focus; both now assert input focus before keyboard input without forcing focus or weakening their closing assertions. The original palette case passed three isolated repetitions on the unchanged base; the initial broader candidate run was 76 passed / 1 failed, and a later pre-final run exposed the same readiness gap in the direct-Escape case (76 passed / 1 failed). Neither failed run is claimed as green.

**Checks.** Node 22.12.0, locked npm ci. Baseline: 348/348 unit/policy tests. Corrected candidate: 364/364 tests across 41 files; production-build-backed Chromium selection: 77/77, two workers, no retries. Selection: `about-profile`, `field-console`, `site-redesign`, `site-runtime`, `hydration-clean`, and `inventory-surface`. Desktop (1280 px) and mobile (390 px) About screenshots were visually inspected. The final commit is requalified after creation, with exact SHA and results recorded in the follow-up PR. Build-generated clinical-trials HTML is excluded from this change.

**Remaining.** Follow-up review, merge, and deployment are separate gates. No new runtime benchmark/MCQ API, viewer implementation, external biographical audit, full repository-wide browser run, or cross-browser certification is claimed. Existing PR #185 threads are left unresolved until the follow-up is integrated; a fresh CodeRabbit result must be read separately from this independent reconciliation.
